...
class MailController extends Controller
{
    ...
    {
        $contacts = Contact_us::orderBy('created_at','DESC')->where('is_sender',0)->where(function ($q) use($request){
            if($request->keyword)
            {
...
//         dd($contacts); toSql()
    ...
    {
        $mail = Contact_us::create($request->all());
    ...
    {
        $contacts = Contact_us::orderBy('created_at','DESC')->where('is_sender',1)->where(function ($q) use($request){
            if($request->keyword)
            {
    ...
    {
        $contacts = Contact_us::onlyTrashed()->orderBy('created_at','DESC')->where(function ($q) use($request){
            if($request->keyword)
            {
